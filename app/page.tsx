...
function GlassCard({ children, className = "", hover = true }: { children: React.ReactNode; className?: string; hover?: boolean }) {
  return (
    <div className={`bg-white/[0.02] backdrop-blur-xl border border-white/[0.06] ${hover ? "hover:bg-white/[0.04] hover:border-white/[0.1] hover:shadow-[0_0_30px_rgba(0,255,65,0.04)]" : ""} transition-all duration-500 ${className}`}>
      {children}
    </div>
  );
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    for (const id of sectionIds) {
      ...
      if (!el) continue;
      const o = new IntersectionObserver((entries) => { for (const e of entries) { if (e.isIntersecting) setActiveSection(id); } }, { threshold: 0.3 });
      o.observe(el);
      obs.push(o);
    }
      ...
      <nav ref={navRef} className="fixed top-0 left-0 right-0 z-50 transition-all duration-500" style={{ background: scrolled ? "rgba(5,5,5,0.85)" : "rgba(5,5,5,0.4)", borderBottom: scrolled ? "1px solid rgba(0,255,65,0.08)" : "1px solid transparent", backdropFilter: scrolled ? "blur(20px)" : "blur(8px)", WebkitBackdropFilter: scrolled ? "blur(20px)" : "blur(8px)" }}>
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navItems.map((item) => (<a key={item} href={`#${item.toLowerCase()}`} className={`nav-link font-mono text-[11px] tracking-[0.15em] transition-colors duration-300 uppercase cursor-pointer ${activeSection === item.toLowerCase() ? "active text-[#00FF41]" : "text-gray-500 hover:text-gray-300"}`}>{item}</a>))}
          </div>
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="md:hidden border-t border-white/[0.06] bg-[#050505]/95 backdrop-blur-2xl overflow-hidden">
              <div className="px-6 py-6 flex flex-col gap-4">
                {navItems.map((item) => (<a key={item} href={`#${item.toLowerCase()}`} onClick={() => setMenuOpen(false)} className={`font-mono text-sm transition-colors cursor-pointer ${activeSection === item.toLowerCase() ? "text-[#00FF41]" : "text-gray-400 hover:text-white"}`}>{item}</a>))}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <main className="relative z-10">
        <section id="home" className="min-h-[100dvh] flex items-center justify-center px-6 pt-16">
          <div className="max-w-4xl w-full">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }} className="mb-6">
              <span className="font-mono text-[11px] tracking-[0.3em] text-gray-600 uppercase inline-flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-[#00FF41]/60 animate-pulse" />Portfolio / 2025</span>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.7 }}>
              <GlassCard className="p-6 mb-8 max-w-2xl rounded-lg" hover={false}>
                <p className="text-gray-300 text-[15px] leading-[1.8]">Building intelligent systems with deep learning, computer vision, and advanced ML algorithms. Currently pursuing B.Tech in Computer Science (AI-ML) while working as a Trainee Software Engineer at Agility Innovations.</p>
              </GlassCard>
            </motion.div>
            ...
            <div className="grid md:grid-cols-[1fr_auto] gap-10 items-start">
              <motion.div variants={fadeUp}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {stats.map((s) => (<GlassCard key={s.label} className="p-3 rounded-lg text-center"><div className="text-xl font-black font-mono leading-none text-gradient-green">{s.value}</div><div className="font-mono text-[9px] text-gray-500 mt-1.5 tracking-[0.1em] uppercase">{s.label}</div></GlassCard>))}
                </div>
              </motion.div>
              <motion.div variants={fadeUp} className="flex justify-center md:justify-end">
                <div className="relative"><Image src="https://raw.githubusercontent.com/iampreetdave-max/portfolio/main/images/profile%20picture.jpeg" alt="Preet Dave" width={200} height={200} className="rounded-lg object-cover border border-white/[0.06] w-40 h-40 md:w-48 md:h-48" /><div className="absolute -inset-1 rounded-lg bg-gradient-to-br from-[#00FF41]/10 to-transparent -z-10" /></div>
              </motion.div>
            </div>
          </motion.div>
        </section>
        ...
        <section id="projects" className="py-20 px-6">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }} variants={stagger} className="max-w-4xl mx-auto">
            ...
            <motion.div variants={stagger} className="grid md:grid-cols-2 gap-4">
              {featuredProjects.map((p) => (<motion.div key={p.id} variants={fadeUp}><ProjectCard project={p} onClick={() => setSelectedProject(p)} /></motion.div>))}
            </motion.div>
            <motion.div variants={fadeUp} className="mt-8 text-center">
              <Link href="/projects" className="group inline-flex items-center gap-2 font-mono text-[12px] tracking-wider border border-white/[0.1] px-8 py-3.5 text-gray-400 hover:border-[#00FF41]/40 hover:text-[#00FF41] transition-all duration-300 rounded-sm cursor-pointer">VIEW ALL PROJECTS <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" /></Link>
            </motion.div>
          </motion.div>
        </section>
            ...
            <div className="space-y-0">
              {experience.map((exp, i) => (
                <motion.div key={i} variants={fadeUp} className="relative pl-10 pb-10 border-l border-white/[0.06] last:pb-0 group">
                  ...
                  <div className="flex flex-wrap items-center gap-3 mb-2"><span className="font-mono text-[11px] text-gray-600 tracking-wider flex items-center gap-1.5"><Calendar size={11} />{exp.period}</span>{exp.active && <span className="font-mono text-[9px] tracking-[0.2em] text-[#00FF41]/70 border border-[#00FF41]/20 bg-[#00FF41]/[0.05] px-2.5 py-0.5 uppercase rounded-sm">Current</span>}</div>
            ...
            <div className="grid md:grid-cols-2 gap-12">
              <motion.div variants={fadeUp}>
                <div className="space-y-4 mb-8">
                  {[{ Icon: Mail, label: "Email", href: "mailto:iampreetdave@gmail.com", text: "iampreetdave@gmail.com" }, { Icon: Phone, label: "Phone", href: "tel:+919081025277", text: "+91 90810 25277" }, { Icon: Github, label: "GitHub", href: "https://github.com/iampreetdave-max", text: "iampreetdave-max" }, { Icon: Linkedin, label: "LinkedIn", href: "https://www.linkedin.com/in/preet-dave-452023271/", text: "preet-dave" }].map((link) => (
              ...
              <motion.div variants={fadeUp}>
                <form name="contact" method="POST" data-netlify="true" action="/success" className="space-y-5">
                  ...
                  {[{ label: "Name", type: "text", name: "name" }, { label: "Email", type: "email", name: "email" }].map((f) => (<div key={f.name}><label className="block font-mono text-[10px] text-gray-600 mb-2 tracking-[0.15em] uppercase">{f.label}</label><input type={f.type} name={f.name} required className="w-full bg-white/[0.02] border border-white/[0.06] p-3.5 text-white text-sm rounded-sm transition-all duration-300 placeholder-gray-800" /></div>))}
                  <div><label className="block font-mono text-[10px] text-gray-600 mb-2 tracking-[0.15em] uppercase">Message</label><textarea name="message" required rows={5} className="w-full bg-white/[0.02] border border-white/[0.06] p-3.5 text-white text-sm rounded-sm transition-all duration-300 resize-none placeholder-gray-800" /></div>
                  <button type="submit" className="w-full font-mono text-[11px] tracking-wider border border-[#00FF41]/60 text-[#00FF41] px-6 py-3.5 hover:bg-[#00FF41] hover:text-black transition-all duration-300 uppercase rounded-sm flex items-center justify-center gap-2 cursor-pointer"><Send size={13} /> Send Message</button>
                </form>
              </motion.div>
            </div>
          </motion.div>
