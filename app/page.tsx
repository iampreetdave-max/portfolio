...
const BOOT_LINES = [
  ...
  { text: "[  OK  ] Preet-AI assistant: llama-3.3-70b via Groq  —  ONLINE", color: "green", delay: 1260 },
  { text: "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", color: "dim", delay: 1380 },
  { text: "All systems NOMINAL. Type 'help' to view available commands.", color: "white", delay: 1520 },
];
...
const PROJECTS = [
  ...
  },
  {
    ...
    category: "ML Engineering · Sports Analytics",
    desc: "3 track-type ensembles. 5 prediction markets. 7,566 historical records. Odds from 9 sportsbooks.",
  ...
  },
  {
    ...
    desc: "110+ sources scraped every 15 min. Audience-specific feeds, smart dedup, 12 auto-tags.",
  ...
  },
  {
    ...
    desc: "TrOCR system converting handwritten notes into searchable knowledge bases with chatbot UI.",
...
const LIVE_SYSTEMS = [
  { name: "sports-prediction-platform.service",  desc: "NBA · Soccer · NASCAR engines — 7 GitHub Actions workflows, autonomous daily runs", uptime: "15+ months",       scale: "10,000+ events" },
  { name: "enterprise-automation-suite.service", desc: "CRM sync · dynamic notifications · cross-platform integrations 24/7",              uptime: "Zero downtime SLA", scale: "8+ active workflows" },
  { name: "ai-intelligence-pipeline.service",    desc: "110+ sources · smart dedup · audience-targeted auto-tagged feeds",                 uptime: "15-min cycles",     scale: "110+ sources" },
  { name: "preet-ai-assistant.service",           desc: "Portfolio AI assistant — llama-3.3-70b via Groq · scoped to Preet's work",         uptime: "Always on",         scale: "type 'ai' to chat" },
];
...
const CMD_HELP = [
  ...
  "│  ls                  │  list sections                             │",
  "├──────────────────────┼───────────────────────────────────────────┤",
  "│  ai                  │  enter AI chat  (llama-3.3-70b via Groq)   │",
  "│  ask <question>      │  quick AI question about Preet             │",
  "│  exit                │  leave AI mode                             │",
  "├──────────────────────┼───────────────────────────────────────────┤",
  "│  clear               │  clear terminal log                        │",
  "└──────────────────────┴───────────────────────────────────────────┘",
  "  ↑ / ↓  navigate history    Tab  autocomplete",
];
...
function Prompt({ isAI = false }: { isAI?: boolean }) {
  if (isAI) {
    return (
      <span className="select-none shrink-0 whitespace-nowrap font-mono">
        <span className="text-[#00D96E]">preet-ai</span>
        <span className="text-[#444]">@</span>
        <span className="text-[#F5A623]">portfolio</span>
        <span className="text-[#444]">:</span>
        <span className="text-[#00D4FF]">~</span>
        <span className="text-[#00D96E] font-bold">›</span>
        <span>&nbsp;</span>
      </span>
    );
  }
...
function SkillBar({ pct, name, items }: { pct: number; name: string; items: string[] }) {
  ...
  return (
    <div ref={ref} className="mb-4">
      <div className="flex items-center gap-2 text-xs">
        ...
        <div className="relative overflow-hidden shrink-0" style={{ width: "160px", height: "1.1em" }}>
          <div className="absolute inset-0 text-[#1A1A1A] whitespace-nowrap overflow-hidden leading-none pt-px" aria-hidden>
            {"░".repeat(28)}
          </div>
          <motion.div
      ...
      <div className="pl-12 flex flex-wrap gap-1 mt-1.5">
        {items.map((item) => (
          <span key={item} className="text-[10px] text-[#3A3A3A] border border-[#1A1A1A] px-1.5 py-px bg-[#0A0A0A] rounded-sm">
            {item}
          </span>
        ))}
      </div>
...
export default function TerminalPortfolio() {
  ...
  // Use ref for AI messages to avoid stale closures
  ...
  useEffect(() => {
    const timers = BOOT_LINES.map((line, i) =>
      setTimeout(() => {
        setBootCount(i + 1);
        if (i === BOOT_LINES.length - 1) setTimeout(() => setBootDone(true), 500);
      }, line.delay)
    );
  ...
  }, [bootCount, bootDone, cmdLog, aiLoading]);

  /* ── Send to AI ── */
  const sendToAI = useCallback(async (question: string) => {
    if (!question.trim() || aiLoading) return;

    setCmdHistory(h => [question, ...h.slice(0, 49)]);
    setHistIdx(-1);
    setAiLoading(true);
    ...
    aiMessagesRef.current = newMessages;

    // Add loading placeholder to log
    setCmdLog(l => [...l, { cmd: question, out: [], isAI: true, aiLoading: true }]);
    ...
    try {
      ...
      const reply: string = data.message ?? "No response.";
      ...
      setCmdLog(prev => {
        ...
        copy[copy.length - 1] = {
          ...copy[copy.length - 1],
          out: ["⚠  AI unavailable. Contact iampreetdave@gmail.com"],
          aiLoading: false,
        };
        return copy;
      });
  ...
  const runCmd = useCallback((raw: string) => {
    ...
    if (cmd === "ai") {
      ...
      setCmdLog(l => [...l, {
        ...
        out: [
          "┌─────────────────────────────────────────────────────┐",
          "│  PREET-AI  ●  Powered by Llama 3.3 via Groq       │",
          "│  Ask me about Preet's skills, projects,          │",
          "│  experience, availability, or just say hi!       │",
    ...
    if (cmd.startsWith("ask ")) {
      ...
      if (q) { sendToAI(q); }
      else { setCmdLog(l => [...l, { cmd: raw, out: ["Usage: ask <your question about Preet>"] }]); }
      return;
    }
    ...
    const RESPONSES: Record<string, string[]> = {
      help: CMD_HELP,
      ls: ["about/", "skills/", `projects/  (${PROJECTS.length} items)`, "experience/", "certs/", "live-systems/", "contact/", "stats/"],
      whoami: [
      ...
      skills: SKILLS.map(s => `${String(s.pct).padStart(3)}%  [${("█".repeat(Math.round(s.pct / 5)) + "░".repeat(20)).slice(0, 20)}]  ${s.name}`),
      projects: PROJECTS.map(p => `${p.status.padEnd(10)}${p.name.padEnd(36)}${p.category}`),
      experience: EXP.map(e => `${e.active ? "● ACTIVE   " : "○ --------- "}${e.role} @ ${e.company}`),
      certs: CERTS.map(c => `${c.verified ? "[✓]" : "[ ]"} ${c.name.padEnd(38)}${c.issuer} (${c.year})`),
      stats: [
        "14+    Projects shipped",
    ...
    const out = RESPONSES[cmd] ?? [`bash: ${cmd}: command not found`, "Type 'help' for available commands."];
  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      if (aiMode) {
        ...
        if (lower === "exit" || lower === "quit" || lower === "exit ai") {
          setAiMode(false);
          setCmdHistory(h => [val, ...h.slice(0, 49)]);
          setHistIdx(-1);
          setCmdLog(l => [...l, { cmd: val, out: ["Left AI mode. Returned to terminal."], isAI: false }]);
        } else if (val) {
          sendToAI(val);
        }
      } else {
      ...
      e.preventDefault();
      if (aiMode) return;
      const cmds = ["help", "ls", "whoami", "about", "skills", "projects", "experience", "certs", "stats", "contact", "ai", "ask ", "clear"];
  ...
  return (
    <div className="min-h-screen bg-[#050505] font-mono text-sm text-[#E2E2E2] flex items-start justify-center py-4 px-4 relative overflow-x-hidden">
      ...
      <div
        ...
        style={{ height: "calc(100vh - 2rem)", boxShadow: "0 0 0 1px #111, 0 24px 80px rgba(0,0,0,0.92), 0 0 80px rgba(0,217,110,0.03)" }}
      >
        {/* Title bar */}
        <div className="flex items-center justify-between bg-[#0C0C0C] border-b border-[#1C1C1C] px-4 py-2.5 shrink-0 select-none">
          ...
          <div className="flex items-center gap-4 text-[#3A3A3A] text-xs">
            <a href="https://github.com/iampreetdave-max" target="_blank" rel="noopener noreferrer" className="hover:text-[#00D96E] transition-colors duration-200">github</a>
            <a href="https://www.linkedin.com/in/preet-dave-452023271/" target="_blank" rel="noopener noreferrer" className="hover:text-[#00D4FF] transition-colors duration-200">linkedin</a>
            <a href="mailto:iampreetdave@gmail.com" className="hover:text-[#F5A623] transition-colors duration-200">email</a>
          </div>
        </div>
        ...
        {aiMode && (
          <motion.div
            ...
            className="bg-[#00D96E]/5 border-b border-[#00D96E]/15 px-4 py-1.5 shrink-0 flex items-center gap-2"
          >
            <span className="text-[#00D96E] animate-pulse text-xs">●</span>
            <span className="text-[#00D96E]/60 text-[10px]">
              PREET-AI ACTIVE · llama-3.3-70b via Groq · scoped to Preet&apos;s portfolio · type &apos;exit&apos; to return
            </span>
          </motion.div>
        )}
        ...
        >
          ...
          <div className="space-y-0.5 mb-1">
            {BOOT_LINES.slice(0, bootCount).map((line, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.12 }}
                className={line.color === "amber" ? "text-[#F5A623] font-semibold" : line.color === "green" ? "text-[#00D96E]" : line.color === "dim" ? "text-[#1E1E1E]" : "text-[#B0B0B0]"}
              >
                {line.text}
              </motion.div>
            ))}
          ...
          {bootDone && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
              ...
              <div className="ml-2 mb-3 border border-[#F5A623]/20 p-4 bg-[#0A0A0A]">
                <div className="text-[#F5A623] font-bold tracking-[0.25em] text-sm mb-1" style={{ textShadow: "0 0 12px rgba(245,166,35,0.4)" }}>
                  P R E E T &nbsp; G H A N S H Y A M &nbsp; D A V E
                </div>
                <div className="text-[#555] text-xs mb-0.5">AI/ML Engineer &nbsp;·&nbsp; Automation Developer &nbsp;·&nbsp; Deep Learning</div>
                <div className="text-[#555] text-xs">B.Tech CS(AI-ML) &nbsp;·&nbsp; Trainee SWE @ Agility Innovations</div>
                <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1.5 text-xs">
                  <span className="text-[#00D96E] font-bold">● Open to AI/ML Roles</span>
                  <span><span className="text-[#444]">loc: </span><span className="text-[#E2E2E2]">Ahmedabad, India</span></span>
                  <a href="mailto:iampreetdave@gmail.com" className="t-link">iampreetdave@gmail.com</a>
                  <a href="https://github.com/iampreetdave-max" target="_blank" rel="noopener noreferrer" className="t-link">github ↗</a>
                  <a href="https://www.linkedin.com/in/preet-dave-452023271/" target="_blank" rel="noopener noreferrer" className="t-link">linkedin ↗</a>
                  <a href="/resume.pdf" download className="t-link">resume.pdf ↓</a>
                </div>
              </div>
              ...
              <div className="ml-2 border-l-2 border-[#1A1A1A] pl-4 mb-1 hover:border-[#F5A623]/30 transition-colors duration-300">
                <p className="text-[#888] leading-relaxed max-w-2xl text-xs">
                  ...
                  With expertise in deep learning, computer vision, and NLP, I develop end-to-end ML pipelines —
                  from data preprocessing and feature engineering to model deployment and optimization.
                </p>
                <p className="text-[#888] leading-relaxed max-w-2xl text-xs mt-2">
                  Currently pursuing B.Tech in CS (AI-ML) while gaining hands-on industry experience as a
                  Trainee Software Engineer, shipping production ML systems validated against live data at scale.
                </p>
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {["Deep Learning", "Computer Vision", "NLP", "MLOps", "Full-Stack", "Automation"].map(tag => (
                    <span key={tag} className="text-[10px] border border-[#00D96E]/20 text-[#00D96E] px-2 py-0.5">{tag}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="ml-2 mb-1 grid grid-cols-2 sm:grid-cols-4 gap-2">
                {[{ v: "14+", l: "Projects" }, { v: "13+", l: "Automations" }, { v: "4,084+", l: "Live Predictions" }, { v: "15mo+", l: "Prod. Uptime" }].map(s => (
                  <div key={s.l} className="stat-card border border-[#1C1C1C] p-3 bg-[#0A0A0A]">
              ...
              <div className="ml-2 mb-1">
                {SKILLS.map(s => <SkillBar key={s.name} pct={s.pct} name={s.name} items={s.items} />)}
              </div>

              {/* PROJECTS */}
              <CmdLine cmd="ls -la projects/" />
              <div className="ml-2 text-[#333] text-[10px] mb-2">total {PROJECTS.length} — click any project to open</div>
                ...
                {PROJECTS.map((p, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between flex-wrap gap-2 mb-1.5">
                      <div className="flex items-center gap-2">
                        <span className={`text-[10px] px-1.5 py-0.5 font-bold border ${p.status === "LIVE" ? "text-[#00D96E] border-[#00D96E]/22 bg-[#00D96E]/5" : "text-[#555] border-[#333]/40"}`}>{p.status}</span>
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {p.tech.slice(0, 5).map(t => <span key={t} className="text-[10px] text-[#333] border border-[#1A1A1A] px-1.5 py-px bg-[#0D0D0D] rounded-sm">{t}</span>)}
                      {p.tech.length > 5 && <span className="text-[10px] text-[#272727]">+{p.tech.length - 5} more</span>}
                    </div>
                    {("roi" in p || "records" in p) && (
                      <div className="flex gap-4 mt-1.5 text-[10px]">
                        {"roi" in p && p.roi && <span><span className="text-[#3A3A3A]">roi: </span><span className="text-[#F5A623] font-bold">{p.roi}</span></span>}
                        {"records" in p && p.records && <span><span className="text-[#3A3A3A]">records: </span><span className="text-[#00D96E]">{p.records}</span></span>}
                      </div>
                    )}
                  </motion.a>
                ))}
              ...
              <div className="ml-2 mb-1 space-y-4">
                {EXP.map((e, i) => (
                  <div key={i} className="border-l-2 border-[#1A1A1A] pl-4 hover:border-[#F5A623]/35 transition-colors duration-300">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      {e.active && <span className="text-[10px] text-[#F5A623] border border-[#F5A623]/22 px-1.5 py-px">ACTIVE</span>}
                      <span className="text-[#E2E2E2] font-bold text-xs">{e.role}</span>
                    </div>
                    <div className="text-[#F5A623] text-xs">{e.company}</div>
                    <div className="text-[#3A3A3A] text-[10px] mt-0.5">{e.period} &nbsp;·&nbsp; {e.location}</div>
                    <div className="text-[#555] text-[11px] mt-1 leading-relaxed max-w-2xl">{e.desc}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="ml-2 mb-1">
                {CERTS.map((c, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs py-1.5 border-b border-[#0D0D0D]">
                    <span className={`shrink-0 ${c.verified ? "text-[#00D96E]" : "text-[#333]"}`}>{c.verified ? "[✓]" : "[ ]"}</span>
              ...
              <div className="ml-2 mb-1 space-y-2">
                {LIVE_SYSTEMS.map((s, i) => (
                  <div key={i} className="text-xs">
                    <div className="flex items-center gap-2">
                      <span className="text-[#00D96E]" style={{ textShadow: "0 0 6px rgba(0,217,110,0.6)" }}>●</span>
                      <span className="text-[#E2E2E2] font-bold">{s.name}</span>
                      <span className="text-[#3A3A3A]">— active (running)</span>
                    </div>
                    <div className="pl-5 text-[#555] text-[10px] mt-0.5">{s.desc}</div>
                    <div className="pl-5 flex gap-5 text-[10px] mt-0.5">
                      <span><span className="text-[#2A2A2A]">uptime: </span><span className="text-[#00D96E]">{s.uptime}</span></span>
                      <span><span className="text-[#2A2A2A]">scale: </span><span className="text-[#F5A623]">{s.scale}</span></span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="ml-2 mb-1 space-y-1.5 text-xs">
                {([
                  ...
                  <div key={k} className="flex items-center gap-3">
                    ...
                    {href
                      ? <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer" download={href === "/resume.pdf" || undefined} className="t-link">{v}</a>
                      : <span className="text-[#E2E2E2]">{v}</span>
                    }
                  </div>
                ))}
                <div className="mt-3 text-[#222] text-[10px]"># Type &apos;help&apos; · &apos;ai&apos; to chat with Preet-AI · ↑↓ history · Tab autocomplete</div>
              </div>
              ...
              <div className="border-t border-[#111] pt-4 mt-6">
                <div className="flex flex-wrap gap-x-6 gap-y-1 text-[10px] text-[#252525]">
                  ...
                  <span>NEXT.JS · TAILWIND · FRAMER MOTION · GROQ</span>
                  <Link href="/projects" className="t-link text-[10px]">all-projects →</Link>
                  <Link href="/fun" className="t-link text-[10px]">fun-zone →</Link>
                  <a href="https://github.com/iampreetdave-max/ai-race-news" target="_blank" rel="noopener noreferrer" className="t-link text-[10px]">ai-race-news ↗</a>
                </div>
              </div>
          ...
          {cmdLog.map((entry, i) => (
            <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4">
              <div className="flex items-center">
                <Prompt isAI={!!entry.isAI} />
                <span className={entry.isAI ? "text-[#00D96E]" : "text-[#E2E2E2]"}>{entry.cmd}</span>
              </div>
              {entry.aiLoading ? (
                <div className="pl-4 mt-1 flex items-center gap-2 text-xs">
                  <span className="text-[#00D96E] animate-pulse">⧙</span>
                  <span className="text-[#3A3A3A]">thinking...</span>
                </div>
              ) : entry.isAI && entry.out.length > 0 ? (
                <div className="ml-4 mt-1 pl-3 border-l border-[#00D96E]/12 py-0.5 space-y-0.5">
                  {entry.out.map((line, j) => (
                    <div key={j} className="text-[#7EC87E] text-xs leading-relaxed">{line || " "}</div>
                  ))}
                </div>
              ) : (
                <div className="pl-4 mt-1 space-y-0.5">
                  {entry.out.map((line, j) => (
                    <div key={j} className="text-[#00D96E] text-xs leading-relaxed">{line || " "}</div>
                  ))}
                </div>
              )}
            </motion.div>
          ...
          {bootDone && (
            <div className="flex items-center mt-4 pt-3 border-t border-[#0D0D0D]">
              ...
              <input
                ...
                onKeyDown={onKeyDown}
                disabled={aiLoading}
                className="flex-1 bg-transparent border-none outline-none text-[#E2E2E2] caret-[#00D96E] font-mono text-sm disabled:opacity-50"
                autoFocus
                autoComplete="off"
                autoCorrect="off"
                autoCapitalize="off"
                spellCheck={false}
                placeholder={aiLoading ? "AI is thinking..." : ""}
